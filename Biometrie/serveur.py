...
# --- Limitation des tentatives pour /verify-face ---
attempts = defaultdict(list)

def check_rate_limit(ethereum_address, max_attempts=10, window_minutes=1):
    now = datetime.utcnow()
    window_start = now - timedelta(minutes=window_minutes)
    
    # Nettoyer les anciennes tentatives
    attempts[ethereum_address] = [t for t in attempts[ethereum_address] if t > window_start]
    
    if len(attempts[ethereum_address]) >= max_attempts:
        return False
    
    attempts[ethereum_address].append(now)
    return True
...
def register():
    try:
        data = request.json
        logging.info("Requête reçue pour /register: %s", data)
        ...
        if not all([ethereum_address, face_image_b64, role, first_name, last_name, consent]):
            logging.error("Données manquantes")
            return jsonify({"success": False, "message": "Données manquantes"}), 400

        if not is_valid_ethereum_address(ethereum_address):
            logging.error("Adresse Ethereum invalide: %s", ethereum_address)
        ...
        if user:
            logging.error("Utilisateur déjà enregistré: %s", ethereum_address)
        ...
        if face_image is None:
            logging.error("Impossible de décoder l'image faciale")
            return jsonify({"success": False, "message": "Impossible de décoder l'image"}), 400

        # Générer l'embedding
        try:
            embedding = DeepFace.represent(face_image, model_name='Facenet', enforce_detection=True)[0]["embedding"]
        except ValueError as e:
            logging.error("Erreur DeepFace: %s", str(e))
            return jsonify({"success": False, "message": "Aucun visage détecté dans l'image"}), 400

        # Crypter et stocker uniquement l'embedding (vecteur)
        encrypted_face = fernet.encrypt(json.dumps(embedding).encode()).decode()
        ...
        logging.info("Utilisateur enregistré avec succès: %s", ethereum_address)
...
# --- Vérification par reconnaissance faciale avec limitation des tentatives ---
@app.route('/verify-face', methods=['POST'])
def verify_face():
    try:
        data = request.json
        logging.info("Requête reçue pour /verify-face: %s", data)
        ethereum_address = data.get("ethereum_address")
        face_image_b64 = data.get("face_image")
        
        if not ethereum_address or not face_image_b64:
            logging.error("Données manquantes: ethereum_address=%s, face_image=%s", ethereum_address, "présent" if face_image_b64 else "absent")
            return jsonify({"success": False, "message": "Données manquantes"}), 400
        ...
        if not check_rate_limit(ethereum_address):
            logging.warning("Trop de tentatives pour %s", ethereum_address)
            return jsonify({"success": False, "message": "Trop de tentatives, réessayez plus tard"}), 429

        user = User.query.filter_by(ethereum_address=ethereum_address).first()
        if not user or not user.face_template:
            logging.error("Utilisateur non trouvé ou non enregistré: %s", ethereum_address)
            return jsonify({"success": False, "message": "Utilisateur non trouvé ou non enregistré"}), 404

        # Décrypter et charger l'embedding stocké
        ...
        try:
            new_embedding_dict = DeepFace.represent(face_image, model_name='Facenet', enforce_detection=True)[0]
            new_embedding = new_embedding_dict["embedding"]
        except ValueError as ve:
            logging.error("Erreur de détection de visage : %s", str(ve))
            return jsonify({"success": False, "message": "Aucun visage détecté dans l'image. Assurez-vous que votre visage est bien visible."}), 400

        # Vérification des types pour débogage
        logging.info("Type de stored_embedding : %s", type(stored_embedding))
        logging.info("Type de new_embedding : %s", type(new_embedding))

        # Calculer la distance euclidienne
        distance = np.linalg.norm(np.array(stored_embedding) - np.array(new_embedding))
        seuil = 0.6 # Ajustez ce seuil selon vos besoins
        if distance < seuil:
            logging.info("Distance entre embeddings: %s, Vérification réussie", distance)
            return jsonify({"success": True, "message": "Visage vérifié avec succès"})
        else:
            logging.info("Distance entre embeddings: %s, Visage non reconnu", distance)
            return jsonify({"success": False, "message": "Les visages ne correspondent pas"}), 401
