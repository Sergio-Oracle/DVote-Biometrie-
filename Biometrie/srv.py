...
def verify_face():
    try:
        ...
        # Décrypter le modèle facial stocké
        ...
        try:
            new_embedding = DeepFace.represent(face_image, model_name='Facenet', enforce_detection=True)[0]["embedding"]
        except ValueError as e:
            logging.error("Erreur DeepFace: %s", str(e))
            return jsonify({"success": False, "message": "Aucun visage détecté dans l'image"}), 400
        ...
        threshold = 0.6  # Ajustez ce seuil selon vos besoins (typique pour Facenet)
        verified = distance <= threshold

        logging.info("Distance entre embeddings: %s, Vérifié: %s", distance, verified)
        response = jsonify({
            "success": verified,
            "message": "Authentification faciale réussie" if verified else "Visage non reconnu",
            "distance": float(distance)  # Pour débogage
        })
        response.headers["Access-Control-Allow-Origin"] = request.headers.get("Origin", "https://localhost:3000")
        return response, 200 if verified else 401
